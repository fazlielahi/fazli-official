...
(function($) {
    ...
    function initCVBuilder(config) {
        ...
        const availableSections = {
            ...
            'languages': {
                ...
                fields: [
                    ...
                    { name: 'proficiency', label: 'Proficiency Level', type: 'text', placeholder: 'e.g., Native, Fluent, Intermediate' }
                ]
            },
            'projects': {
                name: 'Projects',
            ...
            'skills': {
                ...
                fields: [
                    ...
                    { name: 'level', label: 'Proficiency Level (Optional)', type: 'text', placeholder: 'e.g., Advanced, Intermediate' }
                ]
            },
            'references': {
                name: 'References',
        ...
        function collectFormData() {
            try {
                ...
                addedSections.forEach(function(sectionKey) {
                    ...
                    $form.find('input[name^="' + sectionKey + '["], textarea[name^="' + sectionKey + '["]').each(function() {
        ...
        function generateSectionItem(sectionKey, item) {
            ...
            if (sectionKey === 'experience') {
                ...
                html = '<div class="language-item">' +
                    ...
                    (item.proficiency ? ' <span class="language-proficiency">- ' + escapeHtml(item.proficiency) + '</span>' : '') +
                    '</div>';
            ...
            try {
                ...
                if ($photoBox.length > 0) {
                    if (data.photo && data.photo.trim()) {
                        ...
                        const initials = name.charAt(0).toUpperCase() + (name.split(' ')[1] ? name.split(' ')[1].charAt(0).toUpperCase() : 'N');
                ...
                // Update contact info - handle modern template structure properly
                const $contactInfo = $template.find('.contact-info');
                const isModernTemplate = $template.find('.contact-icon').length > 0;
                ...
                if ($contactInfo.length > 0 && isModernTemplate) {
                    ...
                    $contactInfo.find('.contact-item').each(function() {
                        ...
                        // Update address for modern template
                ...
                addedSections.forEach(function(sectionKey) {
                    ...
                    if (validItems.length > 0) {
                        ...
                        if ($section.length === 0) {
                            ...
                            const $summary = $template.find('.summary');
                            if ($summary.length > 0) {
                                $summary.after($newSection);
                            } else {
                                $cvBody.prepend($newSection);
                            }
                        } else {
                            $sectionContent = $section.find('.section-content');
                            if ($sectionContent.length === 0) {
        ...
        setTimeout(function() {
            ...
            const templateClass = cvBuilderConfig.templateSlug === 'classic' ? 'classic' : 'modern';
                ...
                if (cvData[sectionKey] && cvData[sectionKey].length > 0) {
                    ...
                    cvData[sectionKey].forEach(function(item, index) {
                        ...
                        Object.keys(item).forEach(function(field) {
                            const $field = $newEntry.find('input[name="' + sectionKey + '[' + index + '][' + field + ']"], textarea[name="' + sectionKey + '[' + index + '][' + field + ']"]');
                            if ($field.length) {
                        ...
                        const debouncedHandler = debounce(handleFormChange, 300);
                        $newEntry.find('input, textarea').on('input change', debouncedHandler);
                    });
        ...
        function generateSectionFields(sectionKey, sectionConfig) {
            ...
            const debouncedHandler = debounce(handleFormChange, 300);
            $sectionContainer.find('input, textarea').on('input change', debouncedHandler);
        ...
        $('#btn-save-cv').on('click', function() {
            ...
            $.ajax({
                ...
                method: 'POST',
                data: {
                    _token: cvBuilderConfig.csrfToken,
