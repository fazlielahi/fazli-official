...
@section('content')
    ...
    <div class="blogs-section">
        @if($blogs->count() > 0)
            @foreach($blogs->sortByDesc('id') as $blog)
                ...
                <div class="modal fade comment-modal" id="editModal{{ $blog->id }}" tabindex="-1" aria-hidden="true" style="overflow: hidden; padding-right: 0">
                    <div class="modal-dialog modal-fullscreen" style="margin: 0 auto;">
                        <form id="comment-form-{{ $blog->id }}" class="ajax-comment-form" method="POST" action="{{ route('localized.blog.comment', ['lang' => app()->getLocale(), $blog->id]) }}">
                            @csrf
                            <div class="modal-content bg-dark text-light" style="display: flex; flex-direction: column;">
                                                    ...
                                                    @foreach($blog->comments->sortByDesc('created_at') as $comment)
                                                        ...
                                                        @if($user)
                                                            <div class="comment-card">
                                                                <img src="{{ $user && $user->photo ? asset('images/' . $user->photo) : asset('images/default.png') }}" class="user-image">
                                                        ...
                                                        @else
                                                            <div class="comment-card">
                                                                <img src="{{ $comment && $comment->photo ? asset('images/' . $comment->photo) : asset('images/default.png') }}" class="user-image">
                                                                <div class="comment-content">
                                                                    <span class="username">{{ $comment->name }}</span>
                                                                    <span class="timestamp">{{ $comment->created_at->diffForHumans() }}</span>
                                                                    <div class="comment-text">{{ $comment->comment }}</div>
        ...
        @endif
        <div class="ads-section-mobile"></div>
    </div>
